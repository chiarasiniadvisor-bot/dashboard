...
function safeYearFromDateLike(val) {
  ...
  if (y < 1950 || y > 2012) return null; // taglio "outliers"
  return y;
}
...
function normAteneo(raw) {
  ...
  let s = String(raw)
    ...
    .replace(/\s+/g, " ")
    .trim();
  s = s.replace(/, sede di .+$/i, ""); // es. "Modena e Reggio Emilia, sede di Modena"
  return s || null;
}
...
async function fetchContactsAll() {
  ...
  while (true) {
    ...
    while (true) {
      ...
      offset += chunk.length;
      break; // esci dal retry e passa alla pagina dopo
    }
  }
}
...
(async () => {
  ...
  // Usiamo SOLO questi campi (niente PII nel risultato):
  // attributes.ATENEO, attributes.CORSO_ACQUISTATO, attributes.DATA_DI_NASCITA, listIds
  ...
  const out = {
    ...
    datasets: {
      ...
      distribuzione_atenei: atenei,               // [{label:"Bari…",count:...}, …, {label:"Altro…",count:X}]
